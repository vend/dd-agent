...
### Service Discovery
The Datadog Agent now includes support for service discovery. This feature works for Docker containers and can run on platforms such as Kubernetes, Docker Swarm, and Amazon ECS.
...
To run this feature, the only required component on top of the Datadog Agent is a Key/Value store where the configuration templates are defined. Both `etcd` and `consul` are supported for this.

For more info on this feature see the [dedicated wiki page](https://github.com/DataDog/dd-agent/wiki/Service-discovery).

### TCP RTT measurement with `go-metro`
*TODO add link to some usage docs*
...
#### Docker
The dockerized Agent now uses the docker hostname (provided by the `Name` param from `docker info`) as its own hostname when available. This means that for hosts running the dockerized Agent
the reported hostname may change to this docker-provided hostname.
...
#### Zookeeper
The type of the `zookeeper.bytes_received_per_second/bytes_sent_per_second` metrics have been switched from gauge to rate. This will affect the values of these metrics.
...
* [FEATURE] DNS: New check that sends a service check reflecting the status of a hostname's resolution on a nameserver. See [#2249][]
...
* [FEATURE] HDFS: New check collecting metrics on individual data nodes from the JMX-HTTP API of HDFS. See [#2235][], [#2260][] and [#2274][]
...
* [FEATURE] MapReduce: New check collecting metrics on the running Mapreduce jobs. See [#2236][]
...
* [FEATURE] MySQL: Enhance the catalog of metrics reported, and add a service check on the replication state. See [#2116][] and [#2242][] (Thanks [@ovaistariq][], [@zdannar][], [@polynomial][], [@leucos][], [@Zenexer][], [@c960657][], [@nfo][] and [@patricknelson][])
...
* [FEATURE] SSH: Support Windows. **BETA support on Windows** See [#2072][]
...
* [FEATURE] YARN: New check collecting metrics from YARN's ResourceManager REST API. See [#2147][] and [#2207][]
...
* [IMPROVEMENT] Windows: Add uptime metric. See [#2135][]
* [IMPROVEMENT] Windows WMI-based checks (`wmi_check`, System check, IIS, Windows Service, Windows Event Log): gracefully time out WMI queries. See [#2185][] and [#2228][]
...
* [BUGFIX] Zookeeper: Switch `zookeeper.bytes_received_per_second/bytes_sent_per_second` to rates. See [#1848][]
